...
class UnbreakableSeaweed_cont(gym.Env):
    """
    Description: 
    ...
    def __init__(self, foo=0, max_steps=1000, threshold=-1000.0, delay=1, patience=20, cr_threshold=-1000):
        ...
        # Since will be using TD3 algotithm, action space needs to be continous 
        # action space --> motor neurons I2_input, B8a/b, B38, B6/B9/B3, B7
        activity_on = 1
        activity_off = 0
        high_a = np.array([activity_on,  # I2_input
                         activity_on,  # B8a/b
                         activity_on,  # B38
                         activity_on,  # B6/B9/B3
                         activity_on], # B7
                        dtype=np.float32)
        
        low_a = np.array([activity_off,  # I2_input
                         activity_off,  # B8a/b
                         activity_off,  # B38
                         activity_off,  # B6/B9/B3
                         activity_off], # B7
                        dtype=np.float32)
        
        
        self.action_space = gym.spaces.Box(low = low_a,
                                        high = high_a,
                                        dtype = np.float32)
    ...
    def set_training(self, inp):
        # === Verbose Setter ============================================================================================================================================================================================================
        #   tell system if running through env for baseline or training RL
        self.training = inp
        print("training set to", inp)
    ...
    def step(self,action): # return self._state, reward, done, info
        ...
        if self.current_step == self.max_steps:
            ...
            if edible == 1:
                self.fixation_type = 1 # if 0 = unloaded, if 1 = loaded
            else:
        ...
        if self._episode_ended:
            ...
            if self.generat_plots_toggle == 1:
                ...
                self.GeneratePlotsFiles_training_CJF('Plot_'+str(date.today()), 'Trial_IRL_test2/', '__Trained')
    ...
    def MuscleActivations_001(self,action):
        ...
        if self.training:
            # since continous, can go directly from motor activity to pressur/tension
            ...
            self.P_I4=((self.tau_I4*self.P_I4+self.B8*self.TimeStep_h)/(self.tau_I4+self.TimeStep_h))

            ## Update pinch force: If food present, and grasper closed, then approaches
            # pmax pressure as dp/dt=(B8*pmax-p)/tau_p.  Use a quasi-backward-Euler
            self.P_I3_anterior=(self.tau_I3anterior*self.P_I3_anterior+(self.B38+self.B6B9B3)*self.TimeStep_h)/(self.tau_I3anterior+self.TimeStep_h)

            ## Update I3 (retractor) activation: dm/dt=(B6-m)/tau_m
            self.T_I3=(self.tau_I3*self.T_I3+self.TimeStep_h*self.B6B9B3)/(self.tau_I3+self.TimeStep_h)

            ## Update I2 (protractor) activation: dm/dt=(B31-m)/tau_m.  quasi-B-Eul.
            self.T_I2=((self.tau_I2_ingestion*edible+self.tau_I2_egestion*(1-edible))*self.T_I2+self.TimeStep_h*self.B31B32)/((self.tau_I2_ingestion*edible+self.tau_I2_egestion*(1-edible))+self.TimeStep_h)

            ## Update Hinge activation: dm/dt=(B7-m)/tau_m.  quasi-B-Eul.
            # bvec(12,j+1)=(tau_m*hinge_last+dt*B7_last)/(tau_m+dt) # old
            self.T_hinge=(self.tau_hinge*self.T_hinge+self.TimeStep_h*self.B7)/(self.tau_hinge+self.TimeStep_h)
