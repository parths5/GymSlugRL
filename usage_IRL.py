...
if unbreakable:
    # Aplysia Feeding Driver Biting --> Unbreakable = Loaded --> make sure that fixation is 1
    ...
    # Aplysia Feeding Driver Biting --> Breakable = Unloaded --> make sure that fixation is 0
    suffix = str(date.today())
    xlimits = [0,60]
...
## Verify Environment with Stable Baselines
##==========================================
# slug-v0: UnbreakableSeaweed
# slug-v6: UnbreakableSeaweed_ont
...
# training parameters
num_iterations = 1000000 # @param {type:"integer"} or 1M #360000
...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    ...
    def __init__(self, plot_env, eval_env, num_eval_episodes, log_interval: int, eval_interval: int, log_dir: str, verbose=1):
        ...
        self.best_reward = -20 #change depending on performance
...
# Create TD3 model
#model = TD3(MlpPolicy, train_env, verbose=1, learning_rate=.00001, seed = 12) #, seed = 12, random_exploration=1)
...
model = TD3(
    MlpPolicy,
    train_env,
    learning_rate=3e-4,              # GOOD default for TD3
    buffer_size=1_000_000,
    learning_starts=10_000,
    batch_size=256,
    tau=0.005,
    gamma=0.99,
    train_freq=(1, "episode"),       # OR (100, "step") depending on env
    gradient_steps=1,
    policy_delay=2,
...
model = TD3(
    ...
    gamma=0.99,
    tau=0.005,
    train_freq=100,        # MUST be int, NOT tuple
    gradient_steps=100,    # MUST be int
    policy_delay=2,
    target_policy_noise=0.2,
    target_noise_clip=0.5,
    verbose=1,
...
# Train the agent TD3 model with callback class as one of the parameters
